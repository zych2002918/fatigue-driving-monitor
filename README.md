...
> ⚠️ 本仓库为竞赛技术论文的开源复现，不含原参赛队任何未公开代码/固件。原作品为硬件实物（STC89C52 + GY-25 + 薄膜压力传感器等）；本工程以 **PC 软件实现视觉通道、以软件模拟行为通道**，并提供 8051 固件参考（`firmware/`）。本仓库不含论文原文，论文细节以"复现说明"一节对应关系为准。
...
| 验证项 | 结果 |
|---|---|
| 单元测试（EAR/MAR 几何、行为状态机时序、模拟脚本） | ✅ 12/12 通过 |
| 行为通道时序（论文 3.2.7：8s 停转向 → 11s 蜂鸣 → 38s 风扇） | ✅ 实测复现论文规则 |
| 融合决策（30s 窗口事件计数 → 疲劳报警） | ✅ 注入测试通过 |
| GUI 窗口 | ✅ 构造冒烟通过（Windows + wxPython 4.3.1） |
